...
def render_sleep_score():
    # st.header("Sleep Score")
    json_file = "sleep_score.json"

    with st.form("sleep_score_form"):
        st.header("Sleep Score")
        # st.write("Enter sex, age, and sleep duration to calculate the sleep score.")

        sex = st.selectbox("Sex", ["boys", "girls"], key="sleep_sex")
        age = st.number_input("Age (years)", min_value=0.0, max_value=25.0, value=8.0, step=0.1, key="sleep_age")
        sleep_hours = st.number_input("Sleep hours", min_value=0.0, max_value=24.0, value=9.0, step=0.1, key="sleep_hours")

        submitted = st.form_submit_button("Calculate Sleep Score")
...
def render_bp_score():
    ...
    with st.form("bp_score_form"):
        ...
        with col1:
            sex = st.selectbox("Sex", ["boys", "girls"], key="bp_sex")
        ...
        with col2:
            ...
            treated = st.checkbox("Treated", value=False, key="bp_treated")

        submitted = st.form_submit_button("Calculate BP Score")
...
def render_bmi_score():
    # st.header("BMI Score")
    bmi_df = pd.read_csv("bmi.csv")

    with st.form("bmi_score_form"):
        st.header("BMI Score")
        ...
        submitted = st.form_submit_button("Calculate BMI Score")
